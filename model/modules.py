...
def load_graph(filename, load_torso=0, graph_method=None, heart_coarsen=4, torso_coarsen=3):
    hgs, hPs, tgs, tPs = [], [], [], []
    with open(filename + '.pickle', 'rb') as f:
        for i in range(heart_coarsen + 1):
            hgs.append(pickle.load(f))

        for i in range(heart_coarsen):
            hPs.append(pickle.load(f))

        if load_torso == 1:
            for i in range(torso_coarsen + 1):
                tgs.append(pickle.load(f))

            for i in range(torso_coarsen):
                tPs.append(pickle.load(f))
    ...
    if load_torso == 0:
        for i in range(heart_coarsen):
            hP_rev = hPs[i] / hPs[i].sum(axis=0)
            hP_rev = np.transpose(hP_rev)
            hPs_rev.append(torch.from_numpy(hP_rev).float())
            hPs[i] = torch.from_numpy(hPs[i]).float()

        return hgs, hgs, hPs, hPs_rev
    elif load_torso == 1:
        tPs_rev = []
        for i in range(torso_coarsen):
            tP_rev = tPs[i] / tPs[i].sum(axis=0)
            tP_rev = np.transpose(tP_rev)
            tPs_rev.append(torch.from_numpy(tP_rev).float())
            # tPs[i] = torch.from_numpy(tPs[i]).float()
        ...
        if graph_method == 'bipartite':
            ...
            return hgs, hPs, tgs, tPs_rev, Hs, Ps
        else:
            raise NotImplementedError


def get_params(data_path, heart_name, batch_size, load_torso=0, load_physics=0, graph_method=None, heart_coarsen=4, torso_coarsen=3):
    if load_physics == 1:
    ...
    if load_torso == 0:
        hgs, hgs, hPs, hPs_rev = \
            load_graph(graph_file, load_torso, graph_method, heart_coarsen)
    else:
        hgs, hPs, tgs, tPs_rev, Hs, Ps = load_graph(graph_file, load_torso, graph_method, heart_coarsen, torso_coarsen)

    num_nodes = [g.pos.shape[0] for g in hgs]
    print('number of nodes on heart:', num_nodes)

    bgs = []
    for g in hgs:
        g_dataset = HeartEmptyGraphDataset(mesh_graph=g)
        g_loader = DataLoader(g_dataset, batch_size=1, shuffle=True, drop_last=True)
        bg = next(iter(g_loader))
        bgs.append(bg)

    hPs = [hP.to(device) for hP in hPs]
    bgs = [bg.to(device) for bg in bgs]

    if load_torso == 0:
        hPs_rev = [hP.to(device) for hP in hPs_rev]
        ...
        params['g'] = hgs[0]
        for i in range(heart_coarsen):
            params["bg{}".format(i + 1)] = bgs[i + 1]
            params["P{}{}".format(i + 1, i)] = hPs[i]
            params["P{}{}".format(i, i + 1)] = hPs_rev[i]
    elif load_torso == 1:
        t_num_nodes = [t_g.pos.shape[0] for t_g in tgs]
        print('number of nodes on torso:', t_num_nodes)
        t_bgs = []
        for t_g in tgs:
            t_g_dataset = HeartEmptyGraphDataset(mesh_graph=t_g)
            t_g_loader = DataLoader(t_g_dataset, batch_size=1, shuffle=True, drop_last=True)
            t_bg = next(iter(t_g_loader))
            t_bgs.append(t_bg)

        tPs_rev = [tP.to(device) for tP in tPs_rev]
        t_bgs = [t_bg.to(device) for t_bg in t_bgs]

        if graph_method == 'bipartite':
            H_dataset = HeartEmptyGraphDataset(mesh_graph=Hs)
            H_loader = DataLoader(H_dataset, batch_size=1, shuffle=True, drop_last=True)
            ...
            for i in range(heart_coarsen):
                ...
                params["P{}{}".format(i + 1, i)] = hPs[i]
            for i in range(torso_coarsen):
                params["t_bg{}".format(i + 1)] = t_bgs[i + 1]
                params["t_P{}{}".format(i, i + 1)] = tPs_rev[i]
        else:
            raise NotImplementedError
